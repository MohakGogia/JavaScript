...
const Login = (props) => {
  ...
  const [message, setMessage] = useState(""); // Error msg show krane k liye invalid login pr
  let { login } = useContext(AuthContext); //login method le aega firebase ka

  const handleLogin = async (e) => {
    //   email , password se login krao
    try {
      await login(email, password);
      props.history.push("/"); //navigate to /
    } catch (err) {
      setMessage(err.message); // Error msg show krake fields empty hojaenge
      setEmail("");
      setPassword("");
    }
  };

  // Custom CSS dene k liye
